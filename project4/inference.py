...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
      ...
      updated[self.getJailPosition()] = 1
      allPossible = updated
  ...
  def elapseTime(self, gameState):
    ...
    for p in self.legalPositions:
      ...
      for newPos, prob in newPosDist.items():
        new_belief[newPos] += self.beliefs[p] * prob
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for p in self.legalPositions:
      trueDistance = util.manhattanDistance(p, pacmanPosition)
      if emissionModel[trueDistance] > 0:
        allParticle[p] = emissionModel[trueDistance] * beliefs[p]
    allParticle.normalize()

    if allParticle.totalCount() == 0:
      self.initializeUniformly(gameState)
    else:
      self.particles = []
      for i in range(self.numParticles):
        self.particles.append(util.sample(allParticle))
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      "*** YOUR CODE HERE ***"
      newParticle = [util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i]))\
                     for i in range(self.numGhosts)]
    ...
    for particle in self.particles:
      ...
      for i in range(self.numGhosts):
        if noisyDistances[i] is not None:
          weighted_ptc *= emissionModels[i][util.manhattanDistance(pacmanPosition, particle[i])]

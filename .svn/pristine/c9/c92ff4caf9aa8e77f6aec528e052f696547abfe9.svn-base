  ...
  def chooseAction(self, gameState):
    ...
    for distribution in livingGhostPositionDistributions:
      ...
      for p in distribution:
        if distribution[p] > prob:
          prob = distribution[p]
          max_prob_closest = p
      if self.distancer.getDistance(pacmanPosition, max_prob_closest) < closest_distance:
        closest_distance = self.distancer.getDistance(pacmanPosition, max_prob_closest)
        closest_ghost = max_prob_closest

    # choose action
    best_action = None
    min_distence = closest_distance
    for action in legal:

...
def depthFirstSearch(problem):
    ...
    while target!=start:
        solution.insert(0, parents[target][1])
        ...
        if node not in explored:
            ...
            for (nextNode, move, stepCost) in successors:
                if nextNode not in explored:
                    ...
                    parents[(nextNode, move)] = node

    while target != start:
        action = explored[target]
        solution.insert(0, action)
        target = parents[(target, action)]
    return solution
    #util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while frontier.isEmpty() is False:
        ...
        if node not in explored:
            ...
            for (nextNode, move, stepCost) in successors:
                if nextNode not in explored:
                    frontier.push((nextNode, move),
                                  (pathCost[node] + stepCost + heuristic(nextNode, problem)))
                    pathCost[nextNode] = pathCost[node] + stepCost
                    parents[(nextNode, move)] = node
